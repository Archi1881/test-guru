class BadgeService
  ...
  def madness?(count)
    test_passages = TestPassage.where(user: @user, test: @test, passed: false).count

    false if test_passages != count
  end

  def cool?
    @current_test_passages.count == 1 if @test_passage.passed?
  end
  ...
  def count_tests_success(test_ids)
    @user.test_passages.where(test_id: test_ids).passed.uniq.count
  end
